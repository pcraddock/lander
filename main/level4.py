...
def play(screen, clock, difficulty, muted, resource_location, resolution):
    ...
    class Debris(pygame.sprite.Sprite):
        """Object class for dust and debris"""
        def __init__(self):
            ...
            self.rect = self.image.get_rect()
            self.velocity = 6
            self.spawn_midright = [0, 540]
            self.rect.midright = self.spawn_midright
            self.despawn_midleft = [1920, 540]

        def update(self):
            if self.rect.midleft[0] < self.despawn_midleft[0]:
                self.rect.center = [self.rect.center[0]+self.velocity, self.rect.center[1]]
            elif self.rect.midleft[0] >= self.despawn_midleft[0]:
                self.rect.midright = self.spawn_midright
